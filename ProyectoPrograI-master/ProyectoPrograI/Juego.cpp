...
void Juego::jugar() {
    ...
    // Agregar jugadores
    int numJugadores;
    std::cout << "Ingrese el número de jugadores (maximo 7): ";
    std::cin >> numJugadores;
    if (numJugadores < 1 || numJugadores > 7) {
        std::cerr << "Numero de jugadores no valido." << std::endl;
        return;
    }
    ...
    for (int i = 0; i < numJugadores; ++i) {
        ...
        std::cin >> nombre;
        Mano* mano = new Mano();
        JugadorGenerico* jugador = new Jugador(nombre,mano);
    ...
    if (opcion == 'G' || opcion == 'g') {
        ...
        std::cerr << "Opcion no valida. Saliendo del juego.\n";
    }
 ...
 void Juego::cargarPartida(std::string& nombreArchivo) {
    ...
    if (cantidadJugadores < 1 || cantidadJugadores > 6) {
        std::cerr << "Error: Cantidad de jugadores no valida." << std::endl;
        archivo.close();
        return;
    }
    ...
    for (int i = 0; i < cantidadJugadores; ++i) {
        ...
        Mano* mano = new Mano();
        JugadorGenerico* jugador = new Jugador(nombreJugador,mano);
        listaJugadores.insertar(jugador);
